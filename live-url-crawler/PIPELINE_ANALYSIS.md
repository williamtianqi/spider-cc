...
## 9. 第三轮:反爬检测漏抓 + TLS/JA3 指纹伪装(已实施)

### 9.1 问题

1. `async_pipeline_crawler.py`(实测 913 页/s 的最快引擎)**完全没有重试逻辑**:首页一次瞬时 429/超时/连接错误就直接判定"站点抓完",和"这个站本来只有几页"无法区分,当天 1 小时压测存活率 0.2% 的异常根源就在这里。
2. 验证码/WAF 挑战页(Cloudflare "Just a moment..." 等)返回 `200` + 短文本,长度过 50 字符阈值,**被当成正常正文吃进库**,过去 0 检出。
3. 两套引擎都用裸 `urllib`/`aiohttp`:UA 是固定自曝身份的字符串(如 `domain-link-crawler/0.1`),且底层 TLS 握手用 Python `ssl` 默认指纹(JA4 `t12d...` 类),和真实浏览器(`t13d...` 类)在握手层就能被 JA3/JA4 指纹 WAF 区分,与 UA 头内容无关。

### 9.2 修复

- **重试 + 分类**(`domain_link_crawler.py`/`pipeline_domain_crawler.py`/`async_pipeline_crawler.py`):429/5xx/超时/连接错误最多重试 3 次,遵守 `Retry-After`;`CHALLENGE_MARKERS` 命中直接判失败不进正文库;每站 `summary.json` 新增 `likely_blocked` + `error_reasons` 字段,永久性反爬拦截和"站本来就小"现在可区分。已用本地 mock server(429-forever / 429-then-200 / Cloudflare 假页面 三场景)在两套引擎上验证:429-then-200 场景修复前 async 引擎会永久漏站,修复后 `fetched=2 unique=2`。
- **`src/browser_fingerprint.py`(新增)+ `curl_cffi` 集成**:按域名稳定映射到 `chrome136/firefox135/safari184/edge101` 等 `curl_cffi impersonate` profile,真实模拟 TLS/JA3/JA4 + HTTP2 指纹,并自动带上与该指纹匹配的 UA/sec-ch-ua/Accept-Language(比手工维护 UA 池更不容易被"UA 与 TLS 不一致"二次识别)。`curl_cffi` 未安装时两套引擎都会自动降级为 urllib/aiohttp + 按域名轮换的真实浏览器 UA 字符串。已用 `https://tls.browserleaks.com/json` 实测验证:开启前 JA4=`t12d380500...`(脚本特征),开启后 JA4=`t13d1516h2...`(与真实 Chrome 136 一致)。
- 新增 CLI 开关:`async_pipeline_crawler.py` 的 `--impersonate/--no-impersonate`(默认开启)。
- **已知取舍**:本地零延迟压测下 `curl_cffi` AsyncSession 比裸 `aiohttp` 慢约 1.4x(64 并发,4400/s → 3090/s);真实抓取中网络 RTT 占主导,预期影响小于此值,但尚未在真实大规模站点集上重新测过吞吐,需要下一轮压测确认实际影响。
- `requirements.txt` 补充 `aiohttp`(此前缺失)与 `curl_cffi`。

### 9.3 上线前发现的两个环境兼容性 bug(已修复)

写文档时用本机实际会被脚本调用的解释器(裸 `python3` → `/Library/Frameworks/Python.framework/Versions/3.8/bin/python3`,而不是本轮开发一直用来测试的 `/usr/bin/python3`)重新跑了一遍全部验证,发现两个只在该解释器下才会触发的问题,如果不修复会导致 `run_async_crawl_10x.sh`/`run_multiproc_10x.sh` 在真实使用环境里直接崩溃或悄悄误判封锁:

1. **`argparse.BooleanOptionalAction` 在 Python 3.8 不存在**(3.9 才引入),`--impersonate/--no-impersonate` 用了这个 action,导致 `async_pipeline_crawler.py`/`multiproc_async_crawler.py` 在 3.8 解释器下启动即 `AttributeError` 崩溃。改成手写一对 `--impersonate`(`store_true`,默认 `True`)+ `--no-impersonate`(`store_false`,`dest="impersonate"`),兼容所有 Python 3 版本。
2. **`curl_cffi` 不同版本支持的 `impersonate` profile 集合不同**:本机该解释器装的是 `curl_cffi==0.9.0`,没有 `chrome136`/`firefox135`/`safari184`/`safari260`(这些是较新版本才有的 profile);硬编码的 profile 池里这 4 个在 0.9.0 下调用会抛 `ImpersonateError`(是 `RequestsError` 子类,会被重试逻辑吃掉但白白重试 3 次后把健康站点误判为 `likely_blocked`)。修复:`browser_fingerprint.py` 启动时用安装版本自带的 `curl_cffi.requests.impersonate.BrowserType` 枚举反向过滤候选 profile 列表,只保留真正可用的,反射失败或全部不匹配时兜底 `chrome99`/`edge99`(几乎所有版本都支持)。已在 `curl_cffi 0.9.0` 和 `0.13.0` 两个版本上分别跑通全部 mock server 场景 + 真实 `tls.browserleaks.com` JA4 校验。
3. 顺带修复:`run_async_crawl_10x.sh`/`run_multiproc_10x.sh` 固定位置参数之后追加 `"${@:N}"` 转发额外 CLI 参数(此前脚本不转发任何额外参数,写在 README 里的 `--no-impersonate` 用法之前实际不生效)。`requirements.txt` 的 `curl_cffi` 最低版本从 `>=0.13.0` 放宽到 `>=0.9.0`,匹配实际验证过的兼容范围。

这次教训:验证必须用生产实际会用的解释器/依赖版本重跑一遍,不能假设开发时手动 `pip install` 过的某一个 `python3` 就代表全部运行环境。
